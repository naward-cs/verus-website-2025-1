...
const comparisons = [
  {
    ...
    icon: <Server className="h-5 w-5 text-verus-blue" />,
    vm: [
  ...
  {
    title: "Scalability",
    icon: <Scale className="h-5 w-5 text-verus-blue" />,
    vm: [
  ...
  },
  {
    ...
    icon: <Shield className="h-5 w-5 text-verus-blue" />,
    vm: [
  ...
  },
  {
    ...
    icon: <Code className="h-5 w-5 text-verus-blue" />,
    vm: [
  ...
  },
  {
    ...
    icon: <Wallet className="h-5 w-5 text-verus-blue" />,
    vm: [
  ...
  },
  {
    ...
    icon: <Coins className="h-5 w-5 text-verus-blue" />,
    vm: [
  ...
  },
  {
    ...
    icon: <DollarSign className="h-5 w-5 text-verus-blue" />,
    vm: [
...
export function ComparisonTable() {
  return (
    <div className="-mx-4 md:mx-0">
      <div className="w-full overflow-hidden md:rounded-lg border border-[#EFEFEF] bg-white/50 shadow-[0_4px_40px_-12px_rgba(0,0,0,0.1)]">
        {/* Table Header */}
        <div className="grid grid-cols-[1fr,1fr] md:grid-cols-[300px,1fr,1fr] bg-[#F8F9FB]">
          <div className="hidden md:block p-6 md:p-8" />
          <div className="p-6 md:p-8 text-[16px] md:text-[18px] font-medium text-black border-l border-[#EFEFEF]">VM-based Blockchains</div>
          <div className="p-6 md:p-8 text-[16px] md:text-[18px] font-medium text-black border-l border-verus-blue">Verus Protocol</div>
        </div>

        {/* Table Body */}
        {comparisons.map((comparison, index) => (
          <div key={index} className={`grid grid-cols-[1fr,1fr] md:grid-cols-[300px,1fr,1fr] ${index !== 0 ? "border-t border-[#EFEFEF]" : ""}`}>
            {/* Title Column */}
            <div className="col-span-2 md:col-span-1 flex items-center gap-3 p-6 md:p-8">
              <div className="h-8 w-8 rounded-lg bg-verus-blue/10 flex items-center justify-center flex-shrink-0">
                {comparison.icon}
              </div>
              <h3 className="text-[16px] md:text-[18px] font-medium text-black">
                {comparison.title}
              </h3>
            </div>

            {/* VM Content */}
            <div className={`p-6 md:p-8 space-y-2 border-l border-[#EFEFEF] ${index !== 0 ? "border-t border-[#EFEFEF]" : ""}`}>
              {comparison.vm.map((point, i) => (
                <div key={i} className="flex items-start gap-2">
                  <span className="text-black/75 mt-[1px] flex-shrink-0">-</span>
            ...
            {/* Verus Content */}
            <div className={`p-6 md:p-8 space-y-2 border-l border-verus-blue ${index !== 0 ? "border-t border-t-verus-blue" : ""}`}>
      ...
      {/* Conclusion */}
      <div className="mt-4 md:mt-4 w-full md:rounded-lg border border-[#EFEFEF] bg-white/50 p-6 md:p-8 shadow-[0_4px_40px_-12px_rgba(0,0,0,0.1)]">
        <h2 className="text-[18px] md:text-[22px] font-medium text-black mb-4">Conclusion</h2>
        <p className="text-[14px] md:text-[15px] text-black/75 leading-relaxed">
