...
import { verifySignature, VerificationResult } from '@/lib/verificationService';
...
export function VerifiedCryptoAddresses({ 
  ...
  const getSignatureStatuses = (result?: UIVerificationResult) => {
    ...
    const signatures: { sig1Status?: VerificationStatus; sig2Status?: VerificationStatus } = {};
    
    if (result.key1) {
      signatures.sig1Status = result.key1.valid === 'true' ? 'pass' : 
                             result.key1.valid === 'false' ? 'fail' : 'unknown';
    }
    
    if (result.key2) {
      signatures.sig2Status = result.key2.valid === 'true' ? 'pass' : 
                             result.key2.valid === 'false' ? 'fail' : 'unknown';
    }
  ...
  const getExplorerLink = (keyType: string, address?: string) => {
    ...
    if (keyType.toLowerCase().includes('verus') || keyType.toLowerCase().includes('vrsc')) {
  ...
  const displayKeys = showAll ? Object.keys(keys) : Object.keys(keys).slice(0, 3);
      ...
      {selectedKey ? (
          ...
          >
            {verificationResults[selectedKey]?.overall?.valid === 'false' && (
              <div className="text-sm text-red-600 dark:text-red-400 mb-3">
                Warning: Verification failed. The controller of this Verus profile may not control this address.
              </div>
            )}
          </ProofCard>
          <button 
            onClick={() => setSelectedKey(null)}
            className="mt-2 text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 flex items-center"
          >
        ...
        <div className="flex flex-wrap gap-3">
          {displayKeys.map(keyName => {
            ...
            const verificationStatus = getVerificationStatus(verificationResults[keyName]);
            ...
            return (
              <div
                ...
                className="inline-flex items-center rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 px-3 py-1 text-sm cursor-pointer transition-colors gap-2 text-gray-900 dark:text-gray-200"
                title={keys[keyName].address}
              >
                {keyName.toLowerCase().includes('verus') || keyName.toLowerCase().includes('vrsc') ? (
                  <VerusIcon />
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-gray-900 dark:text-gray-200">{formatAddress(keys[keyName].address || '')}</span>
                </div>
          ...
          {!showAll && Object.keys(keys).length > 3 && (
            <button
              onClick={onClickMore}
              className="inline-flex items-center rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 px-3 py-1 text-sm cursor-pointer text-gray-900 dark:text-gray-200"
            >
