...
import { VerusResult, PublicProfileProps } from '@/components/VerusIdProfile/ProfileTypes'
  ...
  const handleSearch = async (verusId: string) => {
    ...
    try {
      ...
      if (verusIdResult.error) {
        addDebugInfo(`Error fetching VerusID: ${JSON.stringify(verusIdResult.error)}`)
        throw new Error(verusIdResult.error.message || 'Failed to fetch VerusID')
      }
      ...
      addDebugInfo(`2. VerusID found: ${verusIdResult.result.identity?.name || 'unnamed'}`)
      
      // Check for contentmap
      if (verusIdResult.result.identity?.contentmap) {
        addDebugInfo(`3. Contentmap found with ${Object.keys(verusIdResult.result.identity.contentmap).length} entries`)
        Object.entries(verusIdResult.result.identity.contentmap).forEach(([key, value]) => {
      ...
      if (verusIdResult.result.identity?.contentmap) {
        ...
        try {
          const profileResult = await FetchVerusProfile(verusIdResult.result.identity)
          
          if (profileResult) {
            addDebugInfo(`5. Profile data successfully retrieved`)
            if (profileResult.public) addDebugInfo(`   - Public profile data found`)
            if (profileResult.services) addDebugInfo(`   - Services found: ${Object.keys(profileResult.services).length}`)
            if (profileResult.keys) addDebugInfo(`   - Keys found: ${Object.keys(profileResult.keys).length}`)
            if (profileResult.collections) addDebugInfo(`   - Collections found: ${Object.keys(profileResult.collections).length}`)
          ...
          addDebugInfo(`Error fetching profile: ${profileError.message || profileError}`)
      ...
      addDebugInfo(`6. Setting profile data ${profileJSON ? 'with' : 'without'} Arweave profile information`)
      ...
      const errorMsg = err.message || 'An error occurred while fetching the VerusID'
      setError(errorMsg)
  ...
  return (
    <div className="w-full md:rounded-lg border border-blue-100/80 dark:border-blue-900/30 bg-white/80 dark:bg-gray-900/50 backdrop-blur-sm p-6 shadow-[0_4px_40px_-12px_rgba(0,0,0,0.1)] dark:shadow-[0_4px_40px_-12px_rgba(0,0,0,0.3)] relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <SearchForm onSearch={handleSearch} isLoading={isLoading} initialValue={searchParams.get('id') || ''} />
