...
const FetchVerusProfile = async (content: Record<string, any>) => {
  ...
  const contentmapValue =
    content.contentmap?.[arweaveVdxfid] ||
  ...
  console.log(
    'FetchVerusProfile: Using public vdxfid:',
  ...
  if (arweaveTxId) {
    ...
    try {
      ...
      console.log(
        'FetchVerusProfile: Fetching Arweave transaction data via API...'
      )
      ...
      console.log(
        'FetchVerusProfile: Raw Arweave data length:',
        arweaveData?.length || 0
      )
      ...
      try {
        ...
        console.log(
          'FetchVerusProfile: Available keys in Arweave data:',
          Object.keys(arweaveJSON)
        )
      } catch (parseError) {
        console.error(
          'FetchVerusProfile: Error parsing Arweave data:',
          parseError
        )
        console.log(
          'FetchVerusProfile: Raw data:',
          arweaveData.substring(0, 200) + '...'
        )
        return null
      }
      ...
      if (arweaveJSON) {
        ...
        if (arweaveJSON.manifest === 'arweave/paths') {
          ...
          if (dataPath) {
            console.log(
              'FetchVerusProfile: Found data path in manifest:',
              dataPath
            )
            ...
            if (!pathResponse.ok) {
              throw new Error(
                `API error for path data: ${pathResponse.statusText}`
              )
            }
            ...
            try {
              ...
              console.log(
                'FetchVerusProfile: Available keys in path data:',
                Object.keys(arweaveJSON)
              )
            } catch (pathError) {
              console.error(
                'FetchVerusProfile: Error parsing path data:',
                pathError
              )
              return null
            }
        ...
        if (!publicData && arweaveJSON.iEXZ3nd4K9fmGDSiQ8J6XLATzUUSKp1eAz) {
          console.log(
            'FetchVerusProfile: Public data found in nested structure'
          )
        ...
        if (!publicData) {
          ...
          console.log(
            'FetchVerusProfile: Available keys:',
            Object.keys(arweaveJSON)
          )
          return null
        }
        ...
        console.log(
          'FetchVerusProfile: Public profile data found, creating profile'
        )
      ...
      console.error(
        'FetchVerusProfile: Error fetching or processing Arweave data:',
        error
      )
    }
