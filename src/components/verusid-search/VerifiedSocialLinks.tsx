...
import {VerificationResult, verifyServiceProof} from '@/lib/verificationService'
  ...
  useEffect(() => {
    ...
    Object.entries(services).forEach(async ([serviceKey, serviceData]) => {
      try {
        ...
        const hasProofData = Object.values(serviceData).some(
          (val) =>
            typeof val === 'string' &&
    ...
    <div className="flex flex-wrap gap-3">
      {Object.entries(services).map(([key, service]) => {
        if (key.toLowerCase().includes('reddit')) {
          console.log(
            `VerifiedSocialLinks: Skipping rendering for Reddit service key: ${key}`
          )
          return null
        }

        // Check if this is a social service with a URL
        const hasUrl = Object.values(service).some(
          (val) => typeof val === 'string' && val.startsWith('http')
        )

        if (!hasUrl) return null

        const url = Object.values(service).find(
          (val) => typeof val === 'string' && val.startsWith('http')
        ) as string
        ...
        if (key.toLowerCase().includes('linkedin')) icon = <LinkedInIcon />
        else if (key.toLowerCase().includes('reddit')) icon = <RedditIcon />
        else if (
          ...
          key.toLowerCase().includes('verus') ||
          key.toLowerCase().includes('vrsc')
        )
          icon = <VerusIcon />
