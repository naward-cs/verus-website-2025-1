  ...
  try {
    ...
    if (data.type === "blockchain") {
      if (!data.address || !data.qualifiedname) {
        console.error("Missing required blockchain data:", {
    ...
    const response = await fetch(`${baseUrl}/api/verificationCheck`, {
      method: "POST",
      headers: {
...
export async function verifyServiceProof(
  ...
  try {
    ...
    const controllerVdxfid =
      ProofsJSON?.controller?.vdxfid || CONTROLLER_VDXFID;
    ...
    const baseUrl =
      window.location.origin || `http://localhost${process.env.PORTF}`;
    const response = await fetch(`${baseUrl}/api/verificationCheck`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
